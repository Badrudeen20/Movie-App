...
module.exports = {
    genre:async function(req,res){
        ...
        const list = await Movie.find({genres:/Action/}).limit(15).skip(startindex).exec()  
        const limit = await Movie.find({genres:/Action/}).countDocuments()
        const rec = await Movie.find({}).sort({_id:-1}).limit(5).exec()
        try{
           
            return res.render('category',{data:list,limit:limit,recent:rec,active:"action",index:indexn})
         }catch(err){
             console.log("fail")
        }
    }
