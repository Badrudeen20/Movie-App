...
module.exports = {
    pakistani:async function(req,res){
        ...
        const list = await Movie.find({category:"Pakistani"}).limit(15).skip(startindex).exec()
        const limit = await Movie.find({category:"Pakistani"}).countDocuments()  
        const rec = await Movie.find({}).sort({_id:-1}).limit(5).exec()
         try{
        
             return res.render('category',{data:list,limit:limit,recent:rec,active:"pakistani",index:indexn})
         }catch(err){
             console.log("fail")
        }
    }
